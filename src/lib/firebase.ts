...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string) {
  ...
  console.warn(`Firestore notice during ${operationType} on ${path}:`, err?.message || err);
  throw error;
}
...
export async function testFirestoreConnection() {
  try {
    ...
    if (error instanceof Error && error.message.includes('the client is offline')) {
      console.warn("Firestore connection notice: Client appears offline. Check Firebase configuration.");
    }
  }
}
